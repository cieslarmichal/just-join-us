...
describe('CategoryRepositoryImpl', () => {
  ...
  describe('Find', () => {
    ...
    it('finds Categories by name', async () => {
      const category = await categoryTestUtils.createAndPersist({ input: { name: 'AI/ML' } });
      await categoryTestUtils.createAndPersist({ input: { name: 'Java' } });

      const foundCategories = await categoryRepository.findCategories({ name: 'ai', page: 1, pageSize: 10 });
  ...
  describe('Count', () => {
    ...
    it('counts Categories by name', async () => {
      await categoryTestUtils.createAndPersist({ input: { name: 'Net' } });
      await categoryTestUtils.createAndPersist({ input: { name: 'Javascript' } });

      const count = await categoryRepository.countCategories({ name: 'jav' });
